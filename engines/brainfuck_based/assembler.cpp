...
class JitFuck : public Brainfuck_Base {
    private:
        ...
        constexpr static unsigned char rig[3] = {0x66, 0xFF, 0xC7}; // inc di
        constexpr static unsigned char lef[3] = {0x66, 0xFF, 0xCF}; // dec di
        constexpr static unsigned char inc[2] = {0xFE, 0x07}; // inc byte ptr[rdi]
        constexpr static unsigned char dec[2] = {0xFE, 0x0F}; // dec byte ptr[rdi]
        constexpr static unsigned char com[6] = {
            0x8A, 0x02, // mov al, byte ptr[rdx] (move input into temporary register)
            0x88, 0x07, // mov byte ptr[rdi], al
            0xFE, 0xC2, // inc dl
        };
        constexpr static unsigned char per[6] = {
            0x8A, 0x07, // mov al, byte ptr[rdi] (move current to temp)
            0x88, 0x01, // mov byte ptr[rcx], al (move temp to output)
            0xFE, 0xC1, // inc cl
        };
        constexpr static unsigned char beg[5] = {
            0xE9, 0x00, 0x00, 0x00, 0x00, // jmp + offset, have to go fill the offset.
        };
        constexpr static unsigned char end[15] = {
